(() => {
  ...
  const ui = {
    ...
    steeringValue: $('comparison-steering-value'), deltaValue: $('comparison-delta-value'), axis: $('comparison-axis-mode')
  };
  ...
  const comparisonDistanceMaximum = (items = selectedLaps()) => isSprintMode(items)
    ? Math.max(1, ...items.map(item => Number(buildDistanceMap(item).at(-1)?.distance) || 0))
    : totalDistance();
  const axisMaximum = (items = selectedLaps()) => state.axisMode === 'time'
    ? Math.max(0.5, ...items.map(item => Number(item.lap.duration) || 0))
    : comparisonDistanceMaximum(items);
  const axisUnit = () => state.axisMode === 'time' ? 's' : 'm';
  ...
  function buildDistanceMap(item) {
    const cacheKey = `${isSprintMode([item]) ? 'sprint' : 'circuit'}|${item.key}`;
    if (state.distanceMapCache.has(cacheKey)) return state.distanceMapCache.get(cacheKey);
    if (isSprintMode([item])) {
      const points = [
        lapPointAtTime(item, item.lap.startTime),
        ...item.session.gpsPoints.filter(point => point.time > item.lap.startTime && point.time < item.lap.endTime).sort((a, b) => a.time - b.time),
        lapPointAtTime(item, item.lap.endTime)
      ].filter(Boolean);
      let distance = 0;
      const map = points.map((point, index) => {
        if (index) distance += gpsDistanceMeters(points[index - 1], point);
        return { time: point.time, distance };
      });
      state.distanceMapCache.set(cacheKey, map);
      return map;
    }
  ...
  function fullComparisonSourceSeries(item, key, sourceHz) {
    const cacheKey = `${state.axisMode}|${item.key}|${key}|${sourceHz}`;
    ...
    item.session.rows.forEach(row => {
      ...
      const x = state.axisMode === 'time' ? time - item.lap.startTime : interpolate(map, time, 'time', 'distance');
  ...
  function visibleComparisonSourceSeries(item, key, sourceHz) {
    ...
    const max = Math.min(state.viewMax ?? axisMaximum(), state.axisMode === 'time' ? item.lap.duration : axisMaximum());
    if (max < min) return [];
  ...
  function sampleLap(item) {
    const cacheKey = `${state.axisMode}|${item.key}`;
    if (state.cache.has(cacheKey)) return state.cache.get(cacheKey);
    const map = buildDistanceMap(item), samples = [];
    const maximum = state.axisMode === 'time' ? item.lap.duration : (Number(map.at(-1)?.distance) || comparisonDistanceMaximum([item]));
    const step = state.axisMode === 'time' ? 0.02 : 1;
    for (let axis = 0; axis <= maximum + step * .25; axis += step) {
      const x = Math.min(maximum, axis);
      const time = state.axisMode === 'time' ? item.lap.startTime + x : interpolate(map, x, 'distance', 'time');
      const row = rowAt(item.session.rows, time);
      samples.push({
        x, elapsed: time - item.lap.startTime,
    ...
    state.cache.set(cacheKey, samples);
    return samples;
  }

  function chartOptions(yTitle) {
    const distance = axisMaximum();
    return {
      responsive: true, maintainAspectRatio: false, animation: false, normalized: false, parsing: false,
      interaction: { mode: 'index', intersect: false },
      plugins: { legend: { display: false }, tooltip: { enabled: true } },
      scales: {
        x: { type: 'linear', min: state.viewMin, max: state.viewMax ?? distance, title: { display: false }, ticks: { display: true, autoSkip: true, maxTicksLimit: 7, padding: 1, font: { size: 8 }, callback: value => `${Number(value).toFixed(state.axisMode === 'time' ? 1 : 0)}${axisUnit()}` } },
  ...
  function setChartViewRange(min, max) {
    const distance = axisMaximum();
    const width = Math.max(state.axisMode === 'time' ? .25 : 2, Math.min(distance, max - min));
  ...
  function jumpToDistance(distance) {
    ...
    const target = Math.max(0, Math.min(axisMaximum(items), Number(distance) || 0));
    const baseline = sampleLap(items[0]);
    const elapsed = state.axisMode === 'time' ? target : interpolate(baseline, target, 'x', 'elapsed');
    setPlaying(false);
  ...
  function previewDistance(distance) {
    ...
    state.hoverDistance = Number.isFinite(target) ? Math.max(0, Math.min(axisMaximum(), target)) : null;
  ...
  function bindChartZoom(canvas) {
    ...
    canvas.addEventListener('wheel', event => {
      ...
      zoomFrame = requestAnimationFrame(() => {
        ...
        const nextWidth = Math.max(state.axisMode === 'time' ? .25 : 2, Math.min(axisMaximum(), (currentMax - currentMin) * factor));
    ...
    canvas.addEventListener('dblclick', () => { state.activeSector = null; state.activeSectorEnd = null; setChartViewRange(0, axisMaximum()); renderSectors(selectedLaps()); renderMap(selectedLaps()); });
  }
  ...
  function renderCharts(items) {
    ...
    rebuildChart('delta', 'comparison-delta-chart', sampled.map(s => ({
      label: s.label, data: s.data.map((point, index) => ({
        x: point.x,
        y: state.axisMode === 'time'
          ? (point.x / Math.max(.001, s.item.lap.duration)) * (s.item.lap.duration - (sampled[0]?.item.lap.duration || 0))
          : point.elapsed - (baseline[index]?.elapsed || 0)
      })),
  ...
  function sectorBounds(items) {
    if (state.axisMode === 'time' && items[0]) {
      const boundaries = lapSectorBoundaries(items[0], items);
      if (boundaries.length >= 2) return boundaries.map(point => point.time - items[0].lap.startTime);
    }
    return [0, ...checkpointDistances(items), comparisonDistanceMaximum(items)];
  }
  ...
  function orderedCheckpointLines(items) {
    ...
    if (isSprintMode(items)) {
      ...
      return passages.map((passage, passageIndex) => ({
        ...
        distance: interpolate(buildDistanceMap(referenceItem), passage.crossing.time, 'time', 'distance')
      }));
    }
  ...
  function lapPointAtTime(item, time) {
    if (Math.abs(time - item.lap.startTime) < .002 && Number.isFinite(item.lap.startLat) && Number.isFinite(item.lap.startLon)) {
      return { time: item.lap.startTime, lat: item.lap.startLat, lon: item.lap.startLon };
    }
    if (Math.abs(time - item.lap.endTime) < .002 && Number.isFinite(item.lap.endLat) && Number.isFinite(item.lap.endLon)) {
      return { time: item.lap.endTime, lat: item.lap.endLat, lon: item.lap.endLon };
    }
  ...
  function sectorPathMetrics(item, sectorIndex, items, commonStart, commonEnd) {
    ...
    const commonDistance = state.axisMode === 'time' ? actualDistance : Math.max(0, commonEnd - commonStart);
    const metrics = {
      actualDistance,
      commonDistance,
  ...
  function selectSectorRange(startIndex, endIndex = startIndex) {
    ...
    if (state.activeSector === null) setChartViewRange(0, axisMaximum(items));
    else {
  ...
  function drawSectorPedalTimeline(dialog, cells, analyses, start, end) {
    ...
    ctx.fillStyle = '#64748b'; ctx.font = '700 8px monospace'; ctx.fillText(`${start.toFixed(state.axisMode === 'time' ? 2 : 0)}${axisUnit()}`, left, height - 3); ctx.textAlign = 'right'; ctx.fillText(`${end.toFixed(state.axisMode === 'time' ? 2 : 0)}${axisUnit()}`, width - right, height - 3); ctx.textAlign = 'left';
  }
  ...
  function openSectorDetail(sectorIndex, comparisonIndex) {
    ...
    dialog.innerHTML = `<div class="sector-detail-shell">
      <header><div><span>CHECKPOINT ANALYSIS</span><h2>S${sectorIndex + 1} 상세 비교</h2><p>${start.toFixed(state.axisMode === 'time' ? 2 : 0)}–${end.toFixed(state.axisMode === 'time' ? 2 : 0)}${axisUnit()} · 첫 번째 선택 랩 기준</p></div><button type="button" data-sector-detail-close aria-label="닫기">×</button></header>
  ...
  function renderSectors(items) {
    ...
    const sectorCaption = index => isSprintMode(items)
      ...
      : `${bounds[index].toFixed(state.axisMode === 'time' ? 2 : 0)}–${bounds[index + 1].toFixed(state.axisMode === 'time' ? 2 : 0)}${axisUnit()}`;
  ...
  function renderMap(items) {
    ...
    const all = items.flatMap(item => [
      lapPointAtTime(item, item.lap.startTime),
      ...item.session.gpsPoints.filter(point => point.time > item.lap.startTime && point.time < item.lap.endTime),
      lapPointAtTime(item, item.lap.endTime)
    ].filter(Boolean));
  ...
  function sampleAtDistance(item, distance) {
    const samples = sampleLap(item);
    if (!samples.length) return null;
    const target = Math.max(0, Math.min(axisMaximum(), distance));
  ...
  function updatePlaybackValues(items, previewAt = state.hoverDistance) {
    ...
    if (ui.deltaValue) ui.deltaValue.innerHTML = playbackValueMarkup(samples.map((sample, index) => {
      ...
      const delta = sample ? (state.axisMode === 'time'
        ? (sample.x / Math.max(.001, items[index].lap.duration)) * (items[index].lap.duration - items[0].lap.duration)
        : sample.elapsed - baselineElapsed) : NaN;
  ...
  function syncPlaybackUi(items = selectedLaps()) {
    ...
    if (ui.playDistance) ui.playDistance.innerHTML = items.length ? items.map((item, index) => {
      ...
      return `<span style="color:${COLORS[index]}">L${item.lap.number} ${(sample?.x || 0).toFixed(state.axisMode === 'time' ? 2 : 1)} ${axisUnit()}</span>`;
    }).join('') : `-- ${axisUnit()}`;
  ...
  function followPlaybackCursors(items) {
    ...
    const distance = axisMaximum(items);
  ...
  function render() {
    const items = selectedLaps();
    if (!state.axisModeChosen && items.length && isSprintMode(items)) state.axisMode = 'time';
    ui.axis?.querySelectorAll('button[data-mode]').forEach(button => button.classList.toggle('active', button.dataset.mode === state.axisMode));
    document.querySelectorAll('[data-comparison-axis-meta]').forEach(element => {
      element.textContent = `${element.dataset.yLabel || ''} · X ${state.axisMode === 'time' ? '랩 경과시간 s' : '중심선 거리 m'}`;
    });
    updateCount(); renderSummary(items); renderCharts(items); renderSectors(items); syncPlaybackUi(items); renderMap(items);
    if (items.length) setStatus(isSprintMode(items)
      ? `${items.length}개 스키드 랩 · ${state.axisMode === 'time' ? '스타트 기준 경과시간' : '스타트 기준 GPS 누적거리'}와 실제 체크포인트 통과 시각으로 비교 중입니다.`
      : `${items.length}개 랩 · ${state.axisMode === 'time' ? '랩 경과시간' : '1 m 간격 공통 중심선 거리'}축으로 비교 중입니다.`);
  }
  window.renderDriverComparison = render;
  window.stopDriverComparisonPlayback = () => setPlaying(false);
  window.updateComparisonTimingMode = (mode, checkpoints = null, startLine = null, finishLine = null) => {
    state.sessions.forEach(session => {
      session.timingMode = mode === 'sprint' ? 'sprint' : 'circuit';
      if (Array.isArray(checkpoints)) {
        session.checkpoints = checkpoints.map(line => line.map(point => ({ ...point })));
      }
      if (Array.isArray(startLine)) session.startLine = startLine.map(point => ({ ...point }));
      if (Array.isArray(finishLine)) session.finishLine = finishLine.map(point => ({ ...point }));
    });
    state.sectorCache.clear();
    state.sectorMetricsCache.clear();
    state.cache.clear();
  ...
  ui.map?.addEventListener('click', event => {
    ...
    if (state.axisMode === 'time' || isSprintMode(selectedLaps())) {
      const item = selectedLaps()[0];
      const candidates = item?.session.gpsPoints.filter(fix => fix.time >= item.lap.startTime && fix.time <= item.lap.endTime) || [];
      const nearest = candidates.reduce((best, fix) => {
        const error = gpsDistanceMeters(point, fix);
        return !best || error < best.error ? { fix, error } : best;
      }, null);
      if (!nearest || nearest.error > 40) return;
      jumpToDistance(state.axisMode === 'time'
        ? nearest.fix.time - item.lap.startTime
        : interpolate(buildDistanceMap(item), nearest.fix.time, 'time', 'distance'));
    } else {
  ...
  ui.axis?.addEventListener('click', event => {
    const button = event.target.closest('button[data-mode]');
    if (!button || button.dataset.mode === state.axisMode) return;
    state.axisMode = button.dataset.mode === 'time' ? 'time' : 'distance';
    state.axisModeChosen = true;
    state.viewMin = 0; state.viewMax = null; state.hoverDistance = null; state.activeSector = null; state.activeSectorEnd = null;
    state.cache.clear(); state.sourceSeriesCache.clear(); state.sectorCache.clear(); state.sectorMetricsCache.clear();
    setPlaying(false); state.playElapsed = 0; render();
  });
